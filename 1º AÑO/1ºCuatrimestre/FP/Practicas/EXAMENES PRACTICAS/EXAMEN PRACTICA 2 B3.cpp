...
int main()
{
	...
	int m[MAX_FIL][MAX_COL] , util_fil , util_col , i , j ,  izda , tmp ;
	bool cambio = true ;
	int v[MAX] , menor , suma ,util_v , fila ;
	...
	do{
		...
		cout << "Introducir numero de elementos del vector ( MAX = elementos de una fila ) ; " ;
		cin >> util_v ;
	}while ((util_v > MAX ) || ( util_v < i )) ;
	...
	for ( izda = 0 ; izda < util_v && cambio ; izda ++ )
	{
		...
		for ( int i = 0 ; i < util_fil ; i++ )  // Inicializacion de menor 
			{
				menor = menor + m[0][i];
				fila = 0 ;
			}
