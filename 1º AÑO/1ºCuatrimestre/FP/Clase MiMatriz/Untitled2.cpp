    ...
    /*	void OrdenarBurbuja()
		{
			...
			cambio = false;
				for (int i = Total_fil()-1 ; i > izda ; i--)
				{
					for (int j = Total_col()-1 ; i > izda ; i--)
					{
					
					if (Dato(i,j) < Dato(i-1,j)
						{  
						cambio = true;
						
						intercambia = Dato(i,j) ; 
						
						dato1 = Dato(i-1,j);	
						
						AsignarDato(i,j,dato1) ;
						
						AsignarDato(i-1,j,intercambia) ; 
						}
					}
				}
			} */
...
int main () {
	...
	MiMatriz matriz , m , constructor;  								/* Es necesario declarar la matriz a usar como una de la clase MiMatriz */
