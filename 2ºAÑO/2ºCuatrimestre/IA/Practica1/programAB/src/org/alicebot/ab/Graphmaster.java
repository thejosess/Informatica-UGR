...
public class Graphmaster {
    ...
    public void addCategory (Category category) {
        ...
        /*
        p= Path.sentenceToPathDOS(inputThatTopic);
        addPath(p, category);
        */


        if( !inputThatTopic.contains("[") && !inputThatTopic.contains("(")) {
            p = Path.sentenceToPath(inputThatTopic);
        }
        else {
            paths = Path.sentenceToPathCorPar(inputThatTopic);
        }

        if(p!=null)
            addPath(p, category);
        else if(paths != null){
            Nodemapper finalNode=new Nodemapper();
            for (int i = 0; i < paths.size(); i++) {
                addPath(paths.get(i),category,finalNode);
            }
        }
    ...
    void addPath(Path path, Category category) {
        ...
        //addPathDOS(root, path, category);

    }

    void addPath(Path path, Category category,Nodemapper finalNode) {
        addPath(root, path, category,finalNode);
    ...
    void addPath(Nodemapper node, Path path, Category category) {
        ...
        if (path == null) {
            ...
            //if (path.word.equals("#") || path.word.equals("^") || (path.word.contains("(") && path.word.contains(")")) ) offset = 0;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    boolean containsNode(Nodemapper node,String word,String caracter){
        ...
        if(word.contains(caracter)){
            if(caracter.equals("["))
                palabras=word.replaceAll("\\[","").replaceAll("]","").split(" ");
            else if(caracter.equals("("))
                palabras=word.replaceAll("\\(","").replaceAll("\\)","").split(" ");

            boolean contains[]=new boolean[palabras.length];

            for (int i = 0; i < palabras.length; i++) {
                if(node.key!=null){
                    if(node.key.equals(palabras[i])){
                        contain = node.key.equals(palabras[i]);
                    }
                }else{
                    if(node.map!=null){
                        if(node.map.containsKey(palabras[i])){
                            contain=node.map.containsKey(palabras[i]);
                            //contains[i]=node.map.containsKey(palabras[i]);
                        }
                    }
                }
            }
    ...
    Nodemapper getNode(Nodemapper node,String word,String caracter){
        ...
        return NodemapperOperator.get(node,key);
    }


    void addPathDOS(Nodemapper node, Path path, Category category) {
        //if (path != null) System.out.println("Enable shortcuts = "+enableShortCuts+" path="+Path.pathToSentence(path)+" "+thatStarTopicStar(path));
        ...
        if (path == null) {
            ...
            if (path.word.startsWith("<SET>")) addSets(path.word, bot, node, category.getFilename());
            Nodemapper nextNode = NodemapperOperator.get(node, path.word);
            addPathDOS(nextNode, path.next, category);
            int offset = 1;
            /////////////////////////////////////////////////////////////////
            if (path.word.equals("#") || path.word.equals("^")) offset = 0;
            //if (path.word.equals("#") || path.word.equals("^") || (path.word.contains("(") && path.word.contains(")")) ) offset = 0;
        ...
        else if (containsNode(node, path.word,"(") &&  path.word.contains("(")){
            if (path.word.startsWith("<SET>")) addSets(path.word, bot, node, category.getFilename());
            //Nodemapper nextNode = NodemapperOperator.get(node, path.word);
            Nodemapper nextNode = getNode(node, path.word,"(");
            node.partentesis=true;
            ...
            if(node.partentesis) {
                addPathDOS(node, path.next, category);
                offset = 0;
                /////////////////////////////////////////////////////////////////
                if (path.word.equals("#") || path.word.equals("^")) offset = 0;
                //if (path.word.equals("#") || path.word.equals("^") || (path.word.contains("(") && path.word.contains(")")) ) offset = 0;
                node.height = Math.min(offset + node.height, node.height);
            }
            ...
            if(path.word.contains("(")){
                ...
                String word=path.word.replace("(","").replace(")","");
                NodemapperOperator.put(node, word, nextNode);
                ...
                String word[]=path.word.replace("[","").replace("]","").split(" ");
                for (int i = 0; i < word.length; i++) {
                    ...
                    if (node.key != null)  {
                     ...
                     if (word[i].startsWith("<SET>")) {
                          addSets(word[i], bot, node, category.getFilename());
                     }

                     NodemapperOperator.put(node, word[i], nextNode);
                     System.out.println("ESO: "+word[i]);
                     ...
                     int offset = 1;
                     /////////////////////////////////////////////////////////////////
                     if (word[i].equals("#") || word[i].equals("^")) offset = 0;
                     //if (path.word.equals("#") || path.word.equals("^") || (path.word.contains("(") && path.word.contains(")"))  ) offset = 0;
                     node.height = Math.min(offset + nextNode.height, node.height);
                }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void addPath(Nodemapper node, Path path, Category category,Nodemapper finalNode) {
        //if (path != null) System.out.println("Enable shortcuts = "+enableShortCuts+" path="+Path.pathToSentence(path)+" "+thatStarTopicStar(path));
        if (path == null) {
            node.category = category;
            node.height = 0;
            ...
            addPath(nextNode, path.next, category,finalNode);
            int offset = 1;
            /////////////////////////////////////////////////////////////////
            if (path.word.equals("#") || path.word.equals("^")) offset = 0;
            //if (path.word.equals("#") || path.word.equals("^") || (path.word.contains("(") && path.word.contains(")")) ) offset = 0;
            ...
            if ( (enableShortCuts && thatStarTopicStar(path.next))  || path.next==null ) {
                nextNode=finalNode;
            }
    ...
    public boolean existsCategory(Category c) {
        if(c.getPattern().contains("(") || c.getPattern().contains("[")){
            return (findNodes(c) !=  null);
        }
    ...
    /**
     *   test if category is already in graph but use the () and []
     *
     *   @return list of nodes wiht category
     */
    public Nodemapper findNodes(Category c) {

        String pattern=c.getPattern();
        while(pattern.contains("(") && pattern.contains(")")) {
            String aux=pattern.substring(0, pattern.indexOf("(")).trim();
            pattern = (aux + pattern.substring(pattern.indexOf(")")+1,pattern.length())).trim();

        }

        while(pattern.contains("[") && pattern.contains("]")) {
            String aux=pattern.substring(0, pattern.indexOf("[")).trim();
            String nextcorchete=pattern.substring(pattern.indexOf("[")+1,pattern.indexOf("]")).trim();
            pattern = (aux+ " " + nextcorchete.split(" ")[0] + pattern.substring(pattern.indexOf("]")+1,pattern.length())).trim();

            String s="";
        }


        return findNode(pattern.trim(), c.getThat(), c.getTopic());
        //return findNode(c.getPattern(), c.getThat(), c.getTopic());
    }
    ...
    final Nodemapper match(Path path, Nodemapper node, String inputThatTopic, String starState, int starIndex, String[] inputStars, String[] thatStars, String[] topicStars, String matchTrace) {
        Nodemapper matchedNode,aux;

        if(node.category!=null){

            //System.out.println(bot.brain.categoryCnt);
            //System.out.println(node.category);
            //System.out.println(bot.brain.getCategories().size());

            //for (int i = 0; i < bot.brain.getCategories().size(); i++) {
               //System.out.println(bot.brain.getCategories().get(i).getPattern());
            //}
            //System.out.println(existsCategory(node.category));

            //Category c = new Category(0, " jajaj <THAT> * <TOPIC> *",  MagicStrings.blank_template, MagicStrings.unknown_aiml_file);
            //System.out.println(existsCategory(c));
            //System.out.println(bot.brain.getCategories().size());
            //System.out.println(findNode(node.category) + " eeee");
            //System.out.println(node + " eeee");
            //System.out.println(existsCategory(c));
            //node.category=null;
        ...
        //else if ((matchedNode = prefixMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null) return matchedNode;
            //else if ((matchedNode = subfixMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null) return matchedNode;
        //else if ((matchedNode = parentesisMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace,"borrar")) != null) return matchedNode;
        else if ((matchedNode = prefixsubfixMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace,"prefix")) != null) return matchedNode;
        else if ((matchedNode = prefixsubfixMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace,"subfix")) != null) return matchedNode;
        //else if ((matchedNode = parentesisMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace,"borrar")) != null) return matchedNode;
        //else if ((matchedNode = braketsMatch(path, node, inputThatTopic, starState, starIndex, inputStars, thatStars, topicStars, matchTrace,"borrar")) != null) return matchedNode;
    ...
    final int existPrefix(String node,String cad){
        int iguales=0;

        int indicePlus=node.indexOf("+");
        if(indicePlus==-1 || indicePlus==0) return -1;
        //plus debe estar en la posicion final del nodo y además la cadena no puede ser menor que la longitud
        // del nodo -1 ya que quitamos el símbolo plus y nos interesa ver si coindciden así
        if(indicePlus==node.length()-1 && (node.length()-1)>cad.length() ) return -1;


        int cont=0;
        for (int i=0;i<indicePlus && iguales!=-1;i++){
            if(node.toUpperCase().charAt(i)!=cad.toUpperCase().charAt(i))
                iguales=-1;
            else cont++;
        }

        if(node.length()-1 != cont) return -1;

        iguales=cont;
        return iguales;
    }

    final int existSubfix(String node,String cad){
        int iguales=0;

        int indicePlus=node.indexOf("+");
        if(indicePlus==-1 || indicePlus!=0) return -1; //no está plus en el nodo o está en otra posición distinta del inicio
        if(indicePlus==0 && (node.length()-1)>cad.length()) return -1;//si plus está al inicio del nodo y la cadena es más pequeña

        int cont=0;
        for (int i=node.length()-1,j=cad.length()-1; i>indicePlus && iguales!=-1;i--,j--){
            if(node.toUpperCase().charAt(i)!=cad.toUpperCase().charAt(j))
                iguales=-1;
            else
                cont++;
        }
    ...
    final Nodemapper prefixsubfixMatch(Path path, Nodemapper node, String input, String starState, int starIndex, String[] inputStars, String[] thatStars, String[] topicStars, String matchTrace,String operador) {


        Nodemapper matchedNode;
        try {
            String word = path.word.toUpperCase();

            if (word.equals("<THAT>")) {starIndex = 0; starState = "thatStar";}
            else if (word.equals("<TOPIC>")) {starIndex = 0; starState = "topicStar";}

            String key="";


            if(node.key!=null) {//si tenemos un nodo interno u hoja, obtenemos la key
                if(operador.equals("prefix")) {
                    if (existPrefix(node.key, word) !=-1) {
                            key = node.key;
                    }
                }
                else  if(operador.equals("subfix")){
                    if (existSubfix(node.key, word)!=-1) {
                        key = node.key;
                    }
                }
            }
            else if(node.map!=null){ //si tenemos un nodo externo //solo si le doy más prioridad que <THAT> y <TOPIC> obtenemos la key
                Iterator<String> it1 = node.map.keySet().iterator();
                int cont=0;
                int min=0;
                while (it1.hasNext()) {
                    String c = it1.next();
                    if(operador.equals("prefix")) {
                        if ((cont=existPrefix(c, word)) !=-1) { //nos quedamos con el que mayor prefijo tenga en comparacion
                            if(cont>min) {
                                key = c;
                                min=cont;
                            }
                        ...
                        if ((cont=existSubfix(c, word)) !=-1) { //nos quedamos con el que mayor prefijo tenga en comparacion
                            if(cont>min) {
                                key = c;
                                min=cont;
                            }
            ...
            if (!key.equals("")){ //si la key ha sido encontrada hacemos match y pasamos al nodo siguiente

                Nodemapper nextNode = NodemapperOperator.get(node,key);

                matchTrace += "[" + node.key + "," + path.word + "]";

                if ((matchedNode = match(path.next, nextNode, input, starState, starIndex, inputStars, thatStars, topicStars, matchTrace)) != null) {
                    return matchedNode;
                }
                else {
                    fail(operador, matchTrace);
                    return null;
                }
            }
            ...
            System.out.println("prefixSubfixMatch: "+Path.pathToSentence(path)+": "+ex);
            ex.printStackTrace();
            return null;
        }

        //ArrayList<String> sets=node.sets;
        //String cadena=node.map.keySet().iterator().next();
        fail(operador, matchTrace);
        return null;
    }
    ...
    final Nodemapper braketsMatch(Path path, Nodemapper node, String input, String starState, int starIndex, String[] inputStars, String[] thatStars, String[] topicStars, String matchTrace,String operador) {
        ...
        try {
            ...
            if(node.key!=null) {//si tenemos un nodo interno u hoja, obtenemos la key
                if(node.key.indexOf('[')==0 && node.key.indexOf(']')==node.key.length()-1){
                    String auxkey=node.key;
                    String palabras[]=auxkey.replace("[","").replace("]","").trim().split(" ");//si uso la v2
                    //String palabras[]=key.replace("[","").replace("]","").trim().split(","); // si uso la v3
                    for (int i=0;i<palabras.length;i++){
                        if(word.equals(palabras[i].toUpperCase())) {
                            key = palabras[i];
                            nodeKey=node.key;
                        }
                    }
                ...
                while (it1.hasNext()) {
                    ...
                    if(c.indexOf('[')==0 && c.indexOf(']')==c.length()-1){
                        ...
                        String palabras[]=c.replace("[","").replace("]","").trim().split(" ");//si uso la v2
                        //String palabras[]=key.replace("[","").replace("]","").trim().split(","); // si uso la v3
                        for (int i=0;i<palabras.length;i++){
                            if(word.equals(palabras[i].toUpperCase())) {
                                key = palabras[i];
            ...
            if (!key.equals("")){ //si la key ha sido encontrada hacemos match y pasamos al nodo siguiente
                ...
                Nodemapper nextNode = NodemapperOperator.get(node,nodeKey);

                matchTrace += "[" + nodeKey + "," + path.word + "]";
            ...
            System.out.println("braketsMatch: "+Path.pathToSentence(path)+": "+ex);
            ex.printStackTrace();
            return null;
        }
    ...
    final Nodemapper parentesisMatch(Path path, Nodemapper node, String input, String starState, int starIndex, String[] inputStars, String[] thatStars, String[] topicStars, String matchTrace,String operador) {
        ...
        try {
            ...
            if(node.key!=null) {//si tenemos un nodo interno u hoja, obtenemos la key
                if(node.key.indexOf('(')==0 && node.key.indexOf(')')==node.key.length()-1){
                    String auxkey=node.key.replace("(","").replace(")","").trim();
                    if(word.equals(auxkey.toUpperCase())) {
                            nodeKey=node.key;
                ...
                while (it1.hasNext()) {
                    ...
                    if(c.indexOf('(')==0 && c.indexOf(')')==c.length()-1){
                        ...
                        String palabra=c.replace("(","").replace(")","").trim();
                        //String palabras[]=key.replace("[","").replace("]","").trim().split(","); // si uso la v3
                        if(word.equals(palabra.toUpperCase())) {
                                nodeKey=c;
                        }
            ...
            if (!nodeKey.equals("")){ //si la key ha sido encontrada hacemos match y pasamos al nodo siguiente
...
//////////////////////////////////// REVISAR NO ESTA BIEN
                Nodemapper nm=null;
                while (it1.hasNext() && !lotengo) {
                    String c = it1.next();

                    if ((nm=node.map.get(c)) != null && (node.key.indexOf("(") == 0 && node.key.indexOf(")") == node.key.length() - 1) ) {
                        if(nm.map!=null) {
                            if (nm.map.containsKey(word))
                                lotengo = true;
                        }else{
                            if(nm.key!=null){
                                if(nm.key.equals(word))
                                {
                                    lotengo=true;
                                }
                            }
                ...
                if (lotengo) {
                    setStars(bot.properties.get(MagicStrings.null_star), starIndex, starState, inputStars, thatStars, topicStars);
                    matchedNode = match(path, nm, input, starState, starIndex, inputStars, thatStars, topicStars, matchTrace);
                    return matchedNode;
                }
                //en caso contrario como la palabra dentro de () puede no aparecer hacemos match con la palabra actual
                // y el nodo siguiente
                else {
                    fail(operador, matchTrace);
                    return null;
                ...
                if(node.key!=null) {
                    if (node.key.indexOf("(") == 0 && node.key.indexOf(")") == node.key.length() - 1) {
                            ...
                            setStars(bot.properties.get(MagicStrings.null_star), starIndex, starState, inputStars, thatStars, topicStars);
                            matchedNode = match(path, node.value, input, starState, starIndex, inputStars, thatStars, topicStars, matchTrace);
                            return matchedNode;
            ...
            System.out.println("parentensisMatch: "+Path.pathToSentence(path)+": "+ex);
            ex.printStackTrace();
            return null;
        }
    ...
    void getCategories(Nodemapper node, ArrayList<Category> categories) {
        if (node == null) return;
            ...
            if (NodemapperOperator.isLeaf(node) || node.shortCut) {
                if (node.category != null) {
                    //añado porque al incluir los parentesis y los corchetes se generan nuevos nodos hojas
                    // pero cada uno tiene la misma category, por lo que si es la misma que no la cuente
