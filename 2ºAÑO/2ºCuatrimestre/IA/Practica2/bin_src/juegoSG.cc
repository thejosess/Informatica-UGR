...
int main(int argc, char ** argv){
  ...
  if (argc < 8){
    ...
    if (mapAux[atoi(argv[4])][atoi(argv[5])] == 'P' and mapAux[atoi(argv[4])][atoi(argv[5])] != 'M')
	    cout << "Posicion (" << argv[4] << "," << argv[5] << ") no es de tipo válido para el jugador" << endl;

    monitor.inicializar(atoi(argv[4]), atoi(argv[5]));
    monitor.setPasos(5000);
    monitor.setRetardo(0);

    // Reviso si es una posicion valida
    if (mapAux[atoi(argv[6])][atoi(argv[7])] == 'P' and mapAux[atoi(argv[6])][atoi(argv[7])] == 'M')
