...
int producir()
{
   ...
   sleep_for( milliseconds( aleatorio<10,200>()) ); //Espera aleatoria
   contador++ ;
...
{
   for ( unsigned int i= 0 ; i < num_items ; i++ ) //num_items = 20
   {
      // producir valor
      int valor_prod = producir();
      // enviar valor
      cout << "Productor va a enviar valor " << valor_prod << endl << flush;
      MPI_Ssend( &valor_prod, 1, MPI_INT, id_buffer, 0, MPI_COMM_WORLD ); //Lo envia de manera sincrona
   }
}
// ---------------------------------------------------------------------
