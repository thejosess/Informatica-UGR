...
void funcion_productor(int numero_orden)
{
	...
	for ( int i= 0 ; i < num_items / num_prod ; i++ )
	{
		...
		MPI_Recv ( &valor_prod, 1, MPI_INT, id_controlador, MPI_ANY_TAG, MPI_COMM_WORLD, &estado );
...
void funcion_controlador()
{
	...
	for ( int i= 0 ; i < num_items * 2 ; i++ )
	{
		...
		MPI_Iprobe( id_controlador, etiq_cons, MPI_COMM_WORLD, &hay_mens, &estado);
		if ( hay_mens ) // si hay mensaje: acepta de consumidores
			etiq_emisor = etiq_cons;
		if ( ! hay_mens ) // si no hay mensaje:
			etiq_emisor = MPI_ANY_TAG ; // aceptar de cualquiera

		// recibe un valor de forma síncrona de un productor o un consumidor
		MPI_Recv( &valor_rec, 1, MPI_INT, id_controlador, etiq_emisor, MPI_COMM_WORLD, &estado );
