...
public:
  ...
  template <class T>
  friend ostream& operator<<(ostream& flujo, const ABB<T> & t);
...
template <class Tbase>
void ABB<Tbase>::ImprimeProfundidad(){
  queue<Nodo> cola;
  Nodo n = arbolb.raiz();
  Nodo aux;
  cola.push(n);
  ...
  while (!cola.empty()){
    n= cola.front();
    cola.pop();
    pila.push(n);//ponemos en la pila
    //ponemos los hijos en la cola
    if((aux = arbolb.derecha(n))!=0)
      cola.push(aux);
    if((aux = arbolb.izquierda(n))!=0)
    cola.push(aux);
  }
  while (!pila.empty()){
    n=pila.top();
    cout<<arbolb.etiqueta(n)<<"profundidad ->  ";
    pila.pop();
  }
}

template <class Tbase>
ostream& operator<<(ostream& flujo, const ABB<Tbase>& t){
  flujo << t.arbolb;
  return flujo;
}
  ...
  if(arbolb.izquierda(n)==nodonulo)
    if(arbolb.derecha(n)==nodonulo){
      ...
      if (padre != nodonulo){
        ...
        ArbolBinario<Tbase> a;
        arbolb.podar_derecha(n, a);
        arbolb.asignar_subarbol(arbolb, a.raiz());
      }
    }
...
typename ABB<Tbase>::const_iterator ABB<Tbase>::begin()const{
  ...
  if(n != nodonulo)
    while(n->izqda != nodonulo)
      n = n->izqda;
  return ABB<Tbase>::const_iterator(n);
}

template <class Tbase>
typename ABB<Tbase>::const_iterator ABB<Tbase>::end()const{
  return const_iterator(nodonulo);
}
