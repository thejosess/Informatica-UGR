...
void Diccionario::addTermino(const Termino & termino)
{
	...
	if (pos == -1){
		cout << "Termino NO encontrado: " << termino.getPalabra() << endl; 
		this->terminos.addElemento(termino);
}
	else
	{
		cout << "Termino ENCONTRADO: " << termino.getPalabra() << endl;
		for(int i = 0; i < termino.getNumDefiniciones(); i++)
			this->terminos[pos].addDefinicion(termino.getDefiniciones()[i]);
	}
}
